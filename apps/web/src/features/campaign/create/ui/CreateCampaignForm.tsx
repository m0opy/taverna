...
export function CreateCampaignForm() {
  ...
  return (
    <form className={styles.form} onSubmit={form.submit}>
      ...
      <fieldset className={styles.coverField}><legend>Обложка</legend><div className={styles.coverGrid}>{coverKeys.map((cover) => <button className={`${styles.coverOption} ${styles[cover]} ${cover === form.coverKey ? styles.selected : ''}`} type="button" key={cover} onClick={() => form.setCoverKey(cover)}><span>✦</span>{coverLabels[cover]}</button>)}</div></fieldset>
